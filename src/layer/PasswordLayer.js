 ...
 */
var PasswordLayer = MyLayer.extend({
    _backGround: null,
    _num0: null,
    _num1: null,
    _num2: null,
    ...
    ctor: function () {
        ...
        this._backGround = new cc.Sprite(res.bg_password_png);
    ...
    loadText: function () {
        ...
        for (var i = 0; i < _password.length; i++) {
            var roomNum = new ccui.Text(_password[i].roomNum + "", "Arial", 42);
            this.addChild(roomNum);
            roomNum.setColor(cc.color(52, 33, 12));
            roomNum.setPosition(cc.p(_password[i].pos.x, _password[i].pos.y));
            roomNum.retain();
            this._passwordNums.push(roomNum);
        }
    },
    loadButton: function () {
        this._close = new MyNode(this, "#icon_close.png", function () {
            ...
            this._password = "";
            for (var i = 0; i < this._passwordNums.length; i++) {
                this._passwordNums[i].setString("一");
            }
            this.getParent()._joinRoomLayer._roomNum = "";
            for (var i = 0; i < this.getParent()._joinRoomLayer._roomNums.length; i++) {
                this.getParent()._joinRoomLayer._roomNums[i].setString("一");
            }
        });
    ...
    onTouchBegan2: function (target, event) {
        if (target.getName() == "清空") {
            ...
            for (var j = this._passwordNums.length - 1; j >= 0; j--) {
                if (this._passwordNums[j].getString() != "一") {
                    this._passwordNums[j].setString("一");
                    break;
                }
            }
        } else {
            for (var i = 0; i < this._passwordNums.length; i++) {
                if (this._passwordNums[i].getString() == "一") {
                    this._passwordNums[i].setString(target.getName());
                    if (i == 5) {
                        this.joinRoom();
                    }
                    break;
    ...
    joinRoom: function () {
        ...
        for (var i = 0; i < this._passwordNums.length; i++) {
            this._password = this._password + this._passwordNums[i].getString();
        }
        cc.log("=======" + this._password);
        Game.op.joinRoom(this.getParent()._joinRoomLayer._roomNum, this._password, function (data) {
            cc.log(data);
    ...
    onExit: function () {
        this._passwordNums.splice(0, this._passwordNums.length);
        this._super();
    },
